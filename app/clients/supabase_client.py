...
def _allowed_transition(old: str, new: str) -> bool:
    m = {
        STATUS_PENDING: {STATUS_ACTIVE, STATUS_CANCELLED, STATUS_EXTENSION_PENDING},
        STATUS_ACTIVE: {STATUS_EXTENSION_PENDING, STATUS_COMPLETED, STATUS_CANCELLED},
        STATUS_EXTENSION_PENDING: {STATUS_ACTIVE, STATUS_CANCELLED},
...
# =========================
# Consents (uno nuevo por solicitud)
# =========================
async def create_consent(item_id: str, buyer_msisdn: str, seller_msisdn: str) -> Dict[str, Any]:
    """
    Inserta SIEMPRE una fila nueva en public.consents.
    Si recibes 409, aún existe algún índice único que lo impide.
    """
    ...
    async with httpx.AsyncClient(timeout=20.0) as c:
        ...
        if r.status_code == 409:
            # No levantar excepción dura; devolvemos detalle para que el caller decida.
            return {"ok": False, "error": "UNIQUE_CONFLICT", "detail": r.text}
        r.raise_for_status()
        rows = r.json()
        return rows[0]
...
async def upsert_consent(item_id: str, buyer_msisdn: str, seller_msisdn: str):
    """Solo por compatibilidad. Ahora siempre crea una fila nueva."""
    return await create_consent(item_id, buyer_msisdn, seller_msisdn)
...
async def get_consent(item_id_or_consent_id: str) -> Optional[Dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        r = await client.get(
            f"{BASE}/consents",
            headers=HEADERS,
            params={"select": "*", "item_id": f"eq.{item_id_or_consent_id}", "order": "id.desc", "limit": 1},
        )
        r.raise_for_status()
        rows = r.json()
        return rows[0] if rows else None
