...
async def add_review(rental_id: int, reviewer_wa: str, rating: int, comment: str) -> Dict[str, Any]:
    """
    Inserta una reseña. Devuelve {"ok": True} en cualquier 2xx, aunque el cuerpo venga vacío o "[]",
    para evitar falsos negativos en el handler.
    """
    async with httpx.AsyncClient(timeout=15.0) as c:
        # Validación de renta y rol
        r_get = await c.get(
            f"{BASE}/rentals",
            headers=HEADERS,
        ...
        # Si tu regla exige status 'completed', deja esto; si no, coméntalo.
