...
def find_category_key(selection_id: str) -> Optional[str]:
    ...
    # Try matching by normalized selection_id directly
...
async def process_interactive_message(from_number: str, interactive_data: Dict):
    ...
    if message_type == "list_reply":
        # WhatsApp may include id and title; be robust
        list_reply = interactive_data.get("list_reply", {}) or {}
        selection_id = list_reply.get("id") or list_reply.get("title") or list_reply.get("payload") or list_reply.get("name")
        ...
        # Try mapping to category key
        mapped_key = find_category_key(selection_id) if selection_id else None
        if mapped_key:
            # Use mapped_key to send questions; pass the generated id (consistent with menus)
            await send_category_questions(from_number, mapped_key)
            return

        # Might be a generated question id
        ...
        button_id = button_reply.get("id") or button_reply.get("title") or button_reply.get("payload")
        ...
        if button_id == "YES":
            ...
            # Could be a question id or text
