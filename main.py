...
def find_category_key(selection_id: str) -> Optional[str]:
    """
    Map selection_id (from menu rows) back to the actual KNOWLEDGE_BASE key.
    Handles:
    - ids generated by _category_to_id
    ...
    # 1) If selection_id is already equal to a category key
    for k in KNOWLEDGE_BASE.keys():
        if selection_id == k:
            return k

    # 2) try to normalize selection_id as if it were generated with _category_to_id
    ...
    # 3) maybe selection_id is the visible title (exact), try matching ignoring accents/case
    norm_sel = _normalize_key(selection_id)
    for k in KNOWLEDGE_BASE.keys():
        if _normalize_key(k) == norm_sel:
            return k

    # 4) fallback: check if selection_id contains a meaningful substring of any key
    for k in KNOWLEDGE_BASE.keys():
        if norm_candidate in _normalize_key(k) or _normalize_key(k) in norm_candidate:
            return k
...
async def send_main_menu(to: str):
    ...
    # Also add the App virtual category at the top (keeps previous UX)
    # Put APP_MAIN first for visibility
    sections = [{
...
async def send_app_submenu(to: str):
    ...
    # choose keys related to app experience (heuristic: REGISTRO, SUSCRIPC, SUSCRIPCIÓN, SUSCRIPCION, RESCATE, POSICIÓN)
    app_keys = []
    for k in KNOWLEDGE_BASE.keys():
        kn = _normalize_key(k)
    ...
    rows = []
    # If none found, fall back to common ids
    if not app_keys:
        app_keys = ["REGISTRO", "SUSCRIPCIÓN", "SUSCRIPCION", "RESCATE", "POSICIÓN"]
...
async def send_category_questions(to: str, category_id: str):
    ...
    if category_id == "APP_GENERAL":
        ...
        # map incoming category_id to the real key
        mapped = find_category_key(category_id)
        if not mapped:
            # also allow if category_id actually equals a display title
            mapped = find_category_key(category_id)
        if not mapped:
    ...
    # Present questions (reply buttons if <=3 else interactive list)
...
async def send_answer(to: str, question_id: str):
    ...
    # 2) maybe the incoming id is actually a normalized question text; try matching by normalized text
    if not qdata:
        norm_in = _normalize_key(question_id)
        for qid, data in QUESTION_ID_MAP.items():
            if _normalize_key(data.get("text", "")) == norm_in:
                qdata = data
                break

    # 3) fallback: maybe the user sent the raw question text and map wasn't created (rare). Search KNOWLEDGE_BASE.
    if not qdata:
    ...
    # 4) another fallback: maybe question_id is an autogenerated style like "PER_CAPITAL::Q1" but QUESTION_ID_MAP was cleared — try regenerate by parsing
    if not qdata and "::Q" in (question_id or ""):
        # try to parse category part
        try:
            ...
            if mapped and mapped in KNOWLEDGE_BASE:
                # regenerate questions for that category temporarily and search index
                ...
                try:
                    ...
                    # also store in QUESTION_ID_MAP for future
                    if qdata:
...
async def process_interactive_message(from_number: str, interactive_data: Dict):
    ...
    if message_type == "list_reply":
        ...
        if selection_id == "APP_MAIN":
            ...
            return
        # Try to map selection to category
        mapped = find_category_key(selection_id)
        if mapped:
            await send_category_questions(from_number, selection_id)
            return
        # Could be a generated question id
        ...
        if button_id == "YES":
            ...
            # button_id may be a question id
