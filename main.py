...
# Configurar logging con nivel DEBUG para máxima visibilidad
...
async def send_main_menu_list(to_msisdn: str) -> Dict[str, Any]:
    ...
    if to_msisdn in conversation_state:
        ...
        logging.debug(f"Estado de conversación limpiado para {to_msisdn}")
...
async def send_questions_list(to_msisdn: str, category_id: str) -> Dict[str, Any]:
    ...
    for question_id, question_data in category["questions"].items():
        ...
        display_title = full_title if len(full_title) <= 24 else full_title[:21] + "..."
        rows.append({
    ...
    logging.debug(f"Estado guardado para {to_msisdn}: categoría {category_id}")
...
async def _post_messages(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            logging.debug(f"Mensaje enviado con éxito a {payload.get('to')}")
            return response.json()
    except httpx.HTTPStatusError as e:
        logging.error(f"Error HTTP al enviar mensaje: {e.response.status_code} - {e.response.text}")
...
async def process_text_message(from_msisdn: str, message_text: str) -> None:
    ...
    logging.debug(f"Procesando mensaje de texto de {from_msisdn}: '{text_clean}'")
    if is_back_command(text_clean):
        logging.debug(f"Usuario {from_msisdn} solicitó volver al menú principal")
        await send_main_menu_list(from_msisdn)
        return
    logging.debug(f"Enviando menú inicial con botones a {from_msisdn}")
...
async def process_interactive_message(from_msisdn: str, interactive_data: Dict[str, Any]) -> None:
    ...
    if interactive_type == "button_reply":
        ...
        logging.debug(f"Usuario {from_msisdn} presionó botón: {button_id} ({button_title})")
        if button_id == "bot_qa":
            ...
            logging.warning(f"ID de botón desconocido: {button_id}")
            await send_initial_menu_with_buttons(from_msisdn)
    elif interactive_type == "list_reply":
        list_reply = interactive_data.get("list_reply", {})
        list_id = list_reply.get("id")
        list_title = list_reply.get("title")
        logging.debug(f"Usuario {from_msisdn} seleccionó de lista: {list_id} ({list_title})")
        if not list_id:
            logging.error("list_id es None o vacío")
            await send_text(from_msisdn, "❌ Error al procesar la selección. Intenta de nuevo.")
            await send_main_menu_list(from_msisdn)
            return
        if list_id.startswith("cat_"):
            category_id = list_id.replace("cat_", "")
            logging.debug(f"list_id recibido: {list_id}")
            logging.debug(f"category_id extraído: {category_id}")
            logging.debug(f"Claves disponibles en QA_CATEGORIZED: {list(QA_CATEGORIZED.keys())}")
            if category_id in QA_CATEGORIZED:
                await send_questions_list(from_msisdn, category_id)
            else:
                logging.error(f"Categoría no encontrada: {category_id}")
                await send_text(from_msisdn, "❌ Categoría no válida. Por favor selecciona una opción del menú.")
                await send_main_menu_list(from_msisdn)
            ...
            try:
                ...
                if len(parts) == 2:
                    ...
                    logging.debug(f"Procesando pregunta - Category: {category_id}, Question: {question_id}")
                    category = QA_CATEGORIZED.get(category_id)
                    if category:
                        question = category["questions"].get(question_id)
                        if question:
                            ...
                            logging.error(f"Pregunta no encontrada: {question_id} en categoría {category_id}")
                            await send_text(from_msisdn, "❌ Pregunta no encontrada.")
                            await send_main_menu_list(from_msisdn)
                    else:
                        logging.error(f"Categoría no encontrada: {category_id}")
                        await send_text(from_msisdn, "❌ Categoría no válida.")
                        await send_main_menu_list(from_msisdn)
                else:
                    logging.error(f"Formato inválido para list_id: {list_id}")
                    await send_text(from_msisdn, "❌ Error al procesar la pregunta seleccionada.")
                    await send_main_menu_list(from_msisdn)
            except Exception as e:
                logging.error(f"Excepción procesando pregunta {list_id}: {e}", exc_info=True)
                await send_text(from_msisdn, "❌ Error al procesar la pregunta seleccionada.")
                await send_main_menu_list(from_msisdn)
        else:
            logging.warning(f"ID de lista desconocido: {list_id}")
            await send_main_menu_list(from_msisdn)
    else:
        logging.warning(f"Tipo de mensaje interactivo desconocido: {interactive_type}")
        await send_initial_menu_with_buttons(from_msisdn)
...
async def verify_webhook(
    ...
    logging.debug(f"Verificando webhook - Mode: {hub_mode}, Token: {hub_verify_token}")
    if hub_mode == "subscribe" and hub_verify_token == VERIFY_TOKEN:
        logging.debug("Verificación de webhook exitosa")
...
async def receive_webhook(request: Request):
    try:
        ...
        logging.debug(f"Webhook recibido: {json.dumps(data, indent=2)}")
        if data.get("object") != "whatsapp_business_account":
            logging.debug("Notificación ignorada - No es de WhatsApp Business")
        ...
        for entry in data.get("entry", []):
            for change in entry.get("changes", []):
                ...
                if not messages:
                    logging.debug("No hay mensajes en esta notificación")
                    continue
                for message in messages:
                    from_msisdn = message.get("from")
                    message_type = message.get("type")
                    message_id = message.get("id")
                    logging.debug(f"Procesando mensaje {message_id} de {from_msisdn} (tipo: {message_type})")
                    if message_type == "interactive":
                        ...
                        logging.debug(f"Mensaje de tipo '{message_type}' recibido - Enviando menú inicial")
                        await send_initial_menu_with_buttons(from_msisdn)
        return Response(status_code=200)
...
if __name__ == "__main__":
    import uvicorn
    logging.info("Iniciando WhatsApp Bot Per Capital con Listas Dinámicas...")
    logging.info(f"Categorías cargadas: {len(QA_CATEGORIZED)}")
    total_questions = sum(len(cat["questions"]) for cat in QA_CATEGORIZED.values())
    logging.info(f"Total de preguntas: {total_questions}")
