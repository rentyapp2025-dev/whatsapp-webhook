...
async def send_main_menu(to_msisdn: str) -> Dict[str, Any]:
    """
    Envía el menú principal de categorías usando un mensaje tipo lista.
    """
    sections = [
        {
    ...
    payload = {
        ...
        "interactive": {
            ...
            "body": {
                "text": "Por favor, selecciona una categoría del siguiente listado:"
            },
            "footer": {
                "text": "Puedes escribir 'volver' en cualquier momento para regresar aquí."
            },
            "action": {
...
async def send_subcategory_menu(to_msisdn: str, category_index: int) -> Dict[str, Any]:
    """
    Envía el submenú de preguntas para una categoría específica usando un mensaje tipo lista.
    """
    category_menu = get_menu_by_category_index(category_index)
    if not category_menu:
        await send_text(to_msisdn, "❌ Categoría no válida. Por favor, selecciona una categoría válida.")
    ...
    payload = {
        ...
        "interactive": {
            ...
            "body": {
                "text": "Selecciona una pregunta del siguiente listado:"
            },
            "footer": {
...
async def process_text_message(from_msisdn: str, message_text: str) -> None:
    ...
    # Detectar si el mensaje es un id de categoría o pregunta
    if text_clean.startswith("category_"):
        try:
            choice = int(text_clean.split("_")[1])
            ...
            if 1 <= choice <= len(QA_CATEGORIZED):
                ...
                await send_text(from_msisdn, f"❌ Opción no válida. Por favor, elige una categoría válida.")
                await send_main_menu(from_msisdn)
        except ValueError:
            await send_text(from_msisdn, "❌ Formato de categoría no válido.")
            await send_main_menu(from_msisdn)
        return

    if text_clean.startswith("question_") and current_category is not None:
        try:
            choice = int(text_clean.split("_")[1])
            ...
            await send_text(from_msisdn, "❌ Formato de pregunta no válido.")
            await send_subcategory_menu(from_msisdn, int(current_category))
        return

    # Si no es un id válido, responder con menú inicial
    logging.info(f"⚠️ Entrada no reconocida de {from_msisdn}: '{text_clean}'")
    if current_category is not None:
        await send_text(from_msisdn, "⚠️ Por favor, selecciona una opción válida del menú.")
        await send_subcategory_menu(from_msisdn, int(current_category))
    else:
            ...
            for change in entry.get("changes", []):
                ...
                for message in messages:
                    ...
                    if message_type == "interactive":
                        # Procesar mensajes interactivos (botones o listas)
